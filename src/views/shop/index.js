...
const Shop = () => {
  useDocumentTitle("Shop | Dharmesh");
  useScrollTop();
              ...
              {isFiltered && (
                <h5>
                  {store.filteredProducts.length > 0 &&
                    `Found ${store.filteredProducts.length} ${
                      store.filteredProducts.length > 1 ? "products" : "product"
                    }`}
                </h5>
              )}
            </div>
            ...
            {({ foundOnBasket }) => (
              <>
                <div
                  ...
                  style={{
                    gridTemplateColumns: `repeat(${columnCount}, 160px)`,
                  }}
                >
                  {store.filteredProducts.length === 0
                    ? new Array(12)
